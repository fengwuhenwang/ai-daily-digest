...
BASE_PATH = os.environ.get('NEWSPAPER_BASE_PATH', '/ai-daily-digest').rstrip('/')
...
def build_project_signal(project, items):
    ...
    aggregate_url = with_base(f'/projects/{project["slug"]}/index.html')
    ...
    if latest:
        activity = {
            ...
            'url': with_base(latest.get('htmlUrl') or aggregate_url),
        }
    else:
        activity = {
            'mode': 'project',
...
def render_portal(projects, editions, generated_at):
    ...
    lead_url = lead_signal['aggregateUrl'] if lead_signal else with_base('/site/index.html')
...
def render_project(project, projects, editions):
    ...
    for p in projects:
        ...
        project_url = with_base(f"/projects/{p['slug']}/index.html")
        switches.append(f'<a class="{cls}" href="{project_url}">{escape(p["label"])}</a>')
    ...
    for idx, item in enumerate(items, start=1):
        ...
        edition_sections.append(
            ...
            f'<iframe class="edition-frame" src="{escape(with_base(item["htmlUrl"]))}" loading="lazy" title="{escape(item["title"])}" data-edition-density="{escape(item["density"])}"></iframe>'
            f'</section>'
        )
    ...
    hero_links.append(f'<a class="preview-link" href="{with_base("/site/index.html")}">项目门户</a>')

    html = PROJECT_TEMPLATE
    replacements = {
        '{{projectLabel}}': escape(project['label']),
        '{{siteIndexUrl}}': escape(with_base('/site/index.html')),
